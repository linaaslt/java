...
<%		
		...
		if (!salis.equals("xxx")){
			...
			kur += kablelis + "`salys`.`id` = " + salis ;
			order_by += kablelis + "`Miestas`";
			kablelis = ",";
		...
		else if (pagal_lyti.equals(1)) {
			order_by += kablelis + "`gyventojai`.`lytis`\n";
			kablelis = ",";
		lentele += order_by;	
		lentele += ",`gyventojai`.`lytis` AS `Lytis`\n"
			+ " FROM `miestai`\n"
			+ " LEFT JOIN `gyventojai` ON (`miestai`.`id` = `gyventojai`.`id_miesto`)\n"
			+ " LEFT JOIN `salys` ON (`miestai`.`id_salies` = `salys`.`id`)\n";
		...
		else if (amziu_ribos.equals("xxx")) {
			lentele += ",`gyventojai`.`amz_grupes` AS `Amziu grupes`";
			order_by += kablelis + "`gyventojai`.`amz_grupes`";
			lentele += " FROM `miestai`\n"
			+ " LEFT JOIN `gyventojai` ON (`miestai`.`id` = `gyventojai`.`id_miesto`)\n"
			+ " LEFT JOIN `salys` ON (`miestai`.`id_salies` = `salys`.`id`)\n";
		lentele += order_by;
